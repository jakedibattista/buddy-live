...
## Phase 6: GEPA Harness Fix and Full Run
...
| `safety_v1` via Vertex | 401 with API key only, causing slow failures rather than the infinite loop |
| ADK bug: `_extract_eval_data` on `score=None` | Crash during GEPA reflection when `safety_v1` is NOT_EVALUATED |

### Fixes

- `evals/run_optimize.py` represents an in-process runner with 60m wall-clock default, logging, and auto `BUDDY_EVAL_FAILURES=1`
- `evals/adk_patches.py` provides a null-score patch for `LocalEvalSampler._extract_eval_data`
- `optimize_config.json` configures `thinking_budget: 0` and `max_metric_calls: 8`
- `optimize_sampler_config.json` configures session-level hallucinations scoring and a 16 turn cap
...
**Decision:** Do **not** merge GEPA `best_prompt.txt` into `prompts.py` because the seed was best, and merging would revert the `vision_coach` and `drill_coach` prompt splits.
...
## Eval Baselines: Running `make eval-failures`

Snapshots live under `services/buddy-live-adk/evals/baselines/` (gitignored logs. Copy key numbers here when re-running).
...
### Post-Phase 5 (`vision_coach` and `drill_coach`)
...
**5/5 still passed.** Alex and Tyler dipped slightly, which was likely due to transfer-boundary noise at the root to `drill_coach` handoff, rather than a functional regression (the happy-path eval log shows correct `transfer_to_agent('drill_coach')` after setup).
...
buddy_live_coach (root)
  ...
  ├─ vision_coach (handles camera checks and warmups)
  ├─ drill_coach (handles captures, analysis, and reviews)
  └─ iq_coach (handles Hockey IQ questions and answers)
```
...
**Learning:** Vision tools belong in a specialist. The root prompt shrank and the framing-struggle eval **improved** from 0.81 to 0.88 post full split.

### Slice 2: `drill_coach`

**When root transfers:** After setup and warm-up, drill readiness through `end_session_recap`.

**Root prompt:** Sections 4 to 6 (drill readiness, scored rep, and recap) moved to `DRILL_COACH_PROMPT`. Reconnect rules were updated: if the session is in a drill or recap phase, it transfers to `drill_coach` rather than root-only recovery.

**Learning:** Keep `evals/agent_module/agent.py` in lockstep with `app/agent.py`. Evals import a parallel `root_agent` and any drift causes false pass or fail results. Run `make test` after every split (67 tests, including tool counts).
...
| **Thin orchestrator-only root** | Diminishing returns, since root already has 4 tools and 3 sub-agents |
| **Workflow graph rewrite** | High effort, requiring staging Cloud Run. Refer to [`ARCHITECTURE.md`](ARCHITECTURE.md) |
...
| **No local Next.js** | Firebase admin keys are not configured locally. We enforce a Vercel-only QA policy |
...
**Problem:** `load_player_memory` matched first name only. Two kids named Alex could get each other's welcome-back lines.
...
## Phase 5: First ADK 2.0 Monitor-to-Improve Loop (2026-05-31)
...
| Player expected a cool-down after the slap shot | Prompt had no cool-down framing. Recap was never positioned as the wind-down |
...
| **Cool-down framing** | Recap now explicitly represents the cool-down. If the player asks for one, the coach gives one easy stretch and then recaps | `app/prompts.py` (`DRILL_COACH_PROMPT` §3) |
| **No-go-silent during analysis** | Explicit instruction to reassure and give a light stretch if the player asks "are we done?" while scoring | `app/prompts.py` (`DRILL_COACH_PROMPT` §2d/2g) |
...
We addressed voice-link churn (ElevenLabs reconnects mid-session) across sessions 2 to 4 below.

---

## Phase 5: Monitor-to-Improve Loop for Sessions 2 to 4 (2026-05-31)
...
### Session `live-tc0ot4sklzju`: Voice Drops and Blank Scorecard (`890578b`)
...
### Session `live-lp7g1qbep1s1`: Silence Drops (`759c7dc` and ElevenLabs Settings)
...
| None | ElevenLabs agent timeouts (owned by us, not code) | Raised "Max conversation duration" 600 to 1800s, "Take turn after silence" 7 to 15s |

### Session `live-yvu3h1au2npn`: Portrait Win and Three New Issues (`237280b`, `34affb8`)
...
The portrait clip was clean and the player *did* take a backhand at about 15.8 seconds, and Gemini's own `coach_summary` says so. But:
...
ends but keep the middle. Owner: Jake. An optional bridge from this repo is to pass a
...
**What we measured:** `make test`, showing 59 passed (11 new phase-guard cases).
...
- `evals/adk_patches.py` provides `apply_vertex_safety_adc_fix()`: ADK's `_VertexAiEvalFacade` prefers `GOOGLE_API_KEY` and builds `vertexai.Client(api_key=...)`, which 401s against the Vertex Gen AI Eval service. The patch (opt-in `BUDDY_SAFETY_VERTEX_ADC=1`) hides the API key only while the facade constructs its client, so the safety judge uses ADC (`puck-buddy` / `us-central1`) while the agent and user simulator keep the Gemini API key. Applied at import in `evals/agent_module/agent.py`.
...
2. **Regression investigation** with two fresh `make eval-failures` runs on the unchanged post-split agent (`2026-06-11-rerun{1,2}-eval-failures.log`):
   ...
   | Returning (Marcus) | None | 0.90 | 1.00 |
...
| --- | --- | --- |
| Voice resilience | Full-session `sendUserActivity()` keepalive (5s), tab-visibility reconnect, and richer `voice_drop` telemetry in `coach_log` | `live-inibrtfoscyy` (drops during silence) |
| ElevenLabs agent | User updated dashboard: `turn_timeout` 60s, `turn_v3`, `max_duration_seconds` 1800, and `silence_end_call_timeout` set to -1 | Same |
| Turn sanitization | Inline `<thought>` tags stripped anywhere in coach text | `live-3gh4vmj133s5` (where the coach spoke its reasoning) |
| Turn dedupe | Extending utterances with real new content is no longer dropped, and results-ready pushes are deduped for 10 minutes | `live-3gh4vmj133s5` and `live-inibrtfoscyy` |
| Post-shot UX | Mandatory cool-down announcement after rep stop, and an unscoreable rep gets one retake via `phase_guard` | `live-utn2frbv3uva` |
| IQ scorecard UI | Study recommendations above breakdown, scroll fix, and hiding camera PiP on IQ recap | `live-3gh4vmj133s5` |
| IQ diagrams | 2-on-1 defender scenario shows both attackers | `live-3gh4vmj133s5` |
| Fresh greet | `load_player_memory` **unwired** from root agent since the tool docstring caused spontaneous "welcome back" greetings | `live-fyg7c9kmng6g` |
| Scorecard review | Announce and consent ("scorecard is ready, walk through it?"), paced 2-step review, and goodbye is gated | `live-fyg7c9kmng6g` (where the player spelled out the desired flow) |
| Reconnect context | `player_name` from Firestore re-sent on voice reconnect (backend restart had dropped "Jake" to "buddy") | `live-fyg7c9kmng6g` (deploy mid-session) |

**What we measured:**

- `make test`, showing **67 passed** after all slices.
- Production health: `buddy-live-adk` `/health` OK; Vercel `/coach` 200 on SHA `27a9491`.
- Cloud Trace: session `live-3gh4vmj133s5` (full IQ recap flow), which is suitable for screenshot.

**What we learned:**

- Tool docstrings are implicit instructions. Indeed, `load_player_memory` fired despite the prompt not calling it. A structural fix (unwiring it) beats prompt hope.
